...
namespace DrapPanel
{
    public partial class LinesForm : Form
    {
        ...
        public LinesForm(string sql)
        {
            InitializeComponent();
            try
            {
                sqlconn.sqlconn(sql, "SQL");
            }
            catch (Exception ex)
            {
                return;
        ...
        private void LinesForm_FormClosed(object sender, FormClosedEventArgs e)
        {//保存数据
            using (StreamWriter sw = new StreamWriter(@"Work\\"+this.Text.Replace('&','_') +"_Lines.lst", false, Encoding.UTF8))
            {
                foreach (Line l in lines)
                {
                    sw.WriteLine(l.dsX_src.ToString()+"\b"+l.dsX_des.ToString()+"\b"+l.srcg_itemIndex.ToString()+"\b"+l.desg_itemIndex.ToString()+"\b"+l.srcg.Name.ToString()+"\b"+l.desg.Name.ToString()+"\b"+l.color.A+","+l.color.R+","+l.color.G+","+l.color.B+"\b"+l.isShow.ToString()+"\b"+l.tag);
                }
            }
            using (StreamWriter sw = new StreamWriter(@"Work\\" + this.Text.Replace('&', '_') + "_Controls.lst", false, Encoding.UTF8))
            {
        ...
        {
            if (File.Exists(@"Work\\" + this.Text.Replace('&', '_') + "_Controls.lst"))
            {
                using (StreamReader sr = new StreamReader(@"Work\\" + this.Text.Replace('&', '_') + "_Controls.lst", Encoding.UTF8))
                {
                    string l = "";
            ...
            {
                string[] ls = this.Text.Split('&');
                GroupBox grp = new GroupBox();
                addNewGroupBox(grp,ls[0] , false);
                grp.Location = new Point(120,80);
                grp = new GroupBox();
                addNewGroupBox(grp, ls[1], false);
                grp.Location = new Point(400, 80);
            }
            if (File.Exists(@"Work\\" + this.Text.Replace('&', '_') + "_Lines.lst"))
            {
                using (StreamReader sr = new StreamReader(@"Work\\" + this.Text.Replace('&', '_') + "_Lines.lst", Encoding.UTF8))
                {
                    string l = "";
                    ...
                    while ((l = sr.ReadLine()) != null)
                    {
                        ...
                        string[] argb = ls[6].Split(',');
                        line.color = Color.FromArgb(int.Parse(argb[0]), int.Parse(argb[1]), int.Parse(argb[2]), int.Parse(argb[3]));
                        if (ls[7].ToLower() == "true")
                        {
                            line.isShow = true;
                        }
                        else line.isShow = false;
                        line.tag = ls[8];
                        lines.Add(line);
                    }
                }
        ...
        private void LinesForm_SizeChanged(object sender, EventArgs e)
        {
            panel4.Height = this.ClientSize.Height - panel1.Height;
        }
